...
| Tasks repository | `features/tasks/data/api_task_repository.dart` | **REUSE / ADAPT** | Implements the **identical** `TaskRepository` interface. `Task`, `TaskRepository` and `TaskController` are byte-identical apart from line endings. Needs import changes only: `asMap`/`parseDay`/`formatDay` live in `core/api/json.dart` in the canonical app, and `isAllDay` already exists in the canonical `task_format.dart`. |
...
| Dashboard | `features/home/data/dashboard_repository.dart`, `home/domain/dashboard.dart` | **REUSE / ADAPT** | A thin `GET /dashboard` + `Dashboard.fromJson`. It's useful for the [[Dashboard]] phase, but the donor `home_page.dart` UI is **DO NOT COPY**. |
...
| Tests | `Fawaz/test/*` (`api_client_test`, `tasks_test`, `onboarding_test`, `widget_test`, `support/`) | **REUSE / ADAPT** (test support) | `support/fake_backend.dart` answers `GET /tasks` and `POST /tasks` but has no per-task `PATCH`/`DELETE` routes. The canonical `test/support/fake_auth_backend.dart` covers auth only. Port the task routes and add the missing ones during [[Phase 3 - Tasks API Integration]]. |
