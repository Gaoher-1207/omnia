...
| D7 | Test count | Re-run: 106 passing at the auth checkpoint; 128 after Phase 3; 146 after Phase 4; 164 after Phase 5A. The README still says 106 (a checkpoint). | Update at the next documentation checkpoint |
| D9 | `Fawaz/backend/tests/test_dashboard.py`, `test_phase2.py` | `test_dashboard_reflects_today` (and three sleep tests in `test_phase2.py`) use the machine's local `date.today()` while the dashboard uses the profile time zone (UTC in tests), so it fails when those dates differ (e.g. just after local midnight east of UTC) | A flaky backend test, not an app bug. Not changed (backend out of scope). |
