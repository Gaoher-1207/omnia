...
/// Tasks stored by the backend (`/api/tasks`). The backend assigns ids, so
...
Task taskFromApi(Map<String, dynamic> json) {
  ...
  if (dueDate != null) {
    ...
    final dueTime = json['due_time'] as String?;
    if (dueTime == null) {
      dueAt = day;
    } else {
...
/// App model → backend body (without id/status; the server owns those).
